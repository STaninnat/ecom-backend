...
func TestHandlersUploadConfigAndS3Config_handleUploadError_AllBranches(t *testing.T) {
	types := []struct {
		name       string
		cfgFactory func(mockLogger *MockLogger) any
	}{
		{
			name: "UploadConfig",
			cfgFactory: func(mockLogger *MockLogger) any {
				return &HandlersUploadConfig{Logger: mockLogger}
			},
		},
		{
			name: "UploadS3Config",
			cfgFactory: func(mockLogger *MockLogger) any {
				return &HandlersUploadS3Config{Logger: mockLogger}
			},
		},
	}
		...
		t.Run(typ.name, func(t *testing.T) {
			for _, tc := range testCases {
				t.Run(tc.name, func(t *testing.T) {
					...
					switch c := cfg.(type) {
					case *HandlersUploadConfig:
						c.handleUploadError(w, req, tc.err, "op", ip, userAgent)
					case *HandlersUploadS3Config:
						c.handleUploadError(w, req, tc.err, "op", ip, userAgent)
					}
